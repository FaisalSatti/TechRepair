...
jQuery( document ).ready( function() {
	...
	// Change view mode
	jQuery( '#trx_addons_theme_panel_section_addons .trx_addons_theme_panel_section_view_mode' )
		.on( 'click', '> span', function(e) {
			var mode = jQuery( this ).data('mode'),
				section = jQuery( '#trx_addons_theme_panel_section_addons' );
				...
				if ( need_update === 0 ) {
					jQuery( '.trx_addons_upgrade_addons' ).after(
						'<div class="trx_addons_info_box trx_addons_info_box">'
							+ TRX_ADDONS_STORAGE['msg_update_addons_result']
						+ '</div>'
					);
