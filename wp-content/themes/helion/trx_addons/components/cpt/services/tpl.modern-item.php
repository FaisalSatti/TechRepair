<?php
/**
 * The style "modern" of the Services item
 *
 * @package WordPress
...
if ( has_post_thumbnail() ) {
	$image = trx_addons_get_attachment_url( 
											...
											apply_filters('trx_addons_filter_thumb_size', trx_addons_get_thumb_size('masonry-big'), 'services-modern')
											);
}
...
?>>
	<div class="sc_services_item_header<?php echo esc_attr($args['hide_bg_image']==1 ? ' without_image' : '') ?>"<?php if ($args['hide_bg_image']==0 && !empty($image)) echo ' style="background-image: url('.esc_url($image).');"'; ?>>
        <div class="sc_services_item_header_inner_top"><?php
            if ($args['featured'] != 'none') {
                if ($args['featured']=='icon' && !empty($meta['icon'])) {
                    ...
                    ?><img src="<?php echo esc_url($meta['image']); ?>" alt="<?php esc_attr_e('Icon', 'helion'); ?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>><?php
                    echo !empty($link)
                        ? '</a>'
                        : '</span>';
		...
		?><div class="sc_services_item_header_inner"><?php
            if  ( !isset($args['show_subtitle']) || $args['show_subtitle']==0 ) {
            ...
            if ( !empty($link) && !empty($args['more_text'] ) && (isset($args['icon_link']) && $args['icon_link']==0 ) ) {
                ?><div class="sc_services_item_button sc_item_button"><a href="<?php echo esc_url($link); ?>" <?php if (!empty($meta['link'])) echo ' target="_blank"'; ?> class="sc_button sc_button_simple"><?php echo esc_html($args['more_text']); ?></a></div><?php
            } else
            if ( !empty($link) && (isset($args['icon_link']) && $args['icon_link']==1 ) ) {?>
                <div class="sc_services_item_button sc_item_button"><a href="<?php echo esc_url($link); ?>" <?php if (!empty($meta['link'])) echo ' target="_blank"'; ?>class="sc_iconic_button"></a></div><?php
            }
		?></div><?php
	?></div><?php
