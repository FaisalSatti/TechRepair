...
$socials_meta = $meta['socials'];
$socials_meta_new = array();
foreach ($socials_meta as $v => $k){
    if($k['url'] != '')
        $socials_meta_new[$k['name']] = $k['url'];
}
...
?>>
	...
	trx_addons_get_template_part('templates/tpl.featured.php',
								...
								apply_filters('trx_addons_filter_args_featured', array(
												...
												'no_links' => empty($link),
												'hover' => !empty($socials_meta_new) ? 'info' : 'zoomin',
												'thumb_size' =>  trx_addons_get_thumb_size('portrait'),
												'post_info' => (!empty($socials_meta_new) || has_excerpt()
																	? '<div class="trx_addons_hover_content">'
                                                                          . (has_excerpt()
                                                                             ? '<div class="sc_team_item_excerpt trx_addons_hover_info">'. get_the_excerpt() .'</div>'
                                                                             : '')
                                                                          . (!empty($socials_meta_new)
