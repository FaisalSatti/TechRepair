<div class="front_page_section front_page_section_features<?php
	$helion_scheme = helion_get_theme_option( 'front_page_features_scheme' );
	if ( ! empty( $helion_scheme ) && ! helion_is_inherit( $helion_scheme ) ) {
		echo ' scheme_' . esc_attr( $helion_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( helion_get_theme_option( 'front_page_features_paddings' ) );
	if ( helion_get_theme_option( 'front_page_features_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$helion_css      = '';
		$helion_bg_image = helion_get_theme_option( 'front_page_features_bg_image' );
...
<?php
	// Add anchor
	$helion_anchor_icon = helion_get_theme_option( 'front_page_features_anchor_icon' );
	$helion_anchor_text = helion_get_theme_option( 'front_page_features_anchor_text' );
...
?>
	...
	<?php
	if ( helion_get_theme_option( 'front_page_features_fullheight' ) ) {
	...
	?>
			...
			<?php
			$helion_css      = '';
			$helion_bg_mask  = helion_get_theme_option( 'front_page_features_bg_mask' );
			$helion_bg_color_type = helion_get_theme_option( 'front_page_features_bg_color_type' );
			if ( 'custom' == $helion_bg_color_type ) {
				$helion_bg_color = helion_get_theme_option( 'front_page_features_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_features_content_wrap content_wrap">
			<?php
			// Caption
			$helion_caption = helion_get_theme_option( 'front_page_features_caption' );
			if ( ! empty( $helion_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_features_caption front_page_block_<?php echo ! empty( $helion_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $helion_caption,'helion_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$helion_description = helion_get_theme_option( 'front_page_features_description' );
			if ( ! empty( $helion_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_features_description front_page_block_<?php echo ! empty( $helion_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $helion_description ),'helion_kses_content' ); ?></div>
				<?php
			}
			...
			if ( is_active_sidebar( 'front_page_features_widgets' ) ) {
				...
				if ( ! helion_exists_trx_addons() ) {
					...
					helion_customizer_need_widgets_message( 'front_page_features_caption', 'ThemeREX Addons - Services' );
				}
			}
			?>
			</div>
