...
?><header class="top_panel top_panel_default
	<?php
	echo ! empty( $helion_header_image ) || ! empty( $helion_header_video ) ? ' with_bg_image' : ' without_bg_image';
...
">
	...
	// Main menu
	if ( helion_get_theme_option( 'menu_style' ) == 'top' ) {
		get_template_part( apply_filters( 'helion_filter_get_template_part', 'templates/header-navi' ) );
	}

	// Mobile header
	if ( helion_is_on( helion_get_theme_option( 'header_mobile_enabled' ) ) ) {
		get_template_part( apply_filters( 'helion_filter_get_template_part', 'templates/header-mobile' ) );
	}

	// Page title and breadcrumbs area
	if ( ! is_single() ) {
		get_template_part( apply_filters( 'helion_filter_get_template_part', 'templates/header-title' ) );
	}
