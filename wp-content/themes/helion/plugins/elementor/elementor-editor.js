...
jQuery( document ).ready(
	function() {
		...
		helion_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		if (window.elementor !== undefined) {
			...
			jQuery( '#elementor-panel' )
				.on( 'input change', '[data-setting*="helion_options_"]', function (e) {
					...
					var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.helion_post_editor' );
					if ( link.length > 0 ) {
						helion_change_post_edit_link_elementor( link );
					}
				} )
				.on( 'click', '.helion_post_editor', function(e) {
					helion_change_post_edit_link_elementor( jQuery(this) );
					if (jQuery(this).hasClass('helion_hidden' )) {
						e.preventDefault();
						return false;
					}
				});
		...
		function helion_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
